...
func printC(_ text: String, color: String = CLIColors.reset) {
...
if CommandLine.arguments[1] == "build" {
    let platforms = ["iOS", "tvOS", "xrOS", "watchOS", "macOS"]

    printC("Build \(productName) for all platforms...")

    for platform in platforms {
        printC("Building \(productName) on \(platform).\r")
        let process = Process()
        process.launchPath = "/usr/bin/env"
        process.arguments = [
            "xcrun",
        ...
        if process.terminationStatus != 0 {
            printC("Failed to build for \(platform)", color: CLIColors.red)
            exit(3)
        } else {
            printC("Build for \(platform) successful", color: CLIColors.green)
        }
    }
}
